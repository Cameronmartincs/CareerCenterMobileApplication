...
public class DialogBubble : MonoBehaviour {
		...
		foreach (PixelBubble vBubble in vcharacter.vBubble)
		{
			...
			if (vcharacter.vCurrentBubble == null)
			{
				...
				if (vBubble.vMessageForm == BubbleType.Rectangle)
				{
					...
					vBubbleObject.transform.position = vcharacter.transform.position + new Vector3(25.35f, 25.9f, 0f); //move a little bit the teleport particle effect
				}
				else 
				{
					//create bubble
					vBubbleObject = Instantiate(Resources.Load<GameObject> ("Customs/BubbleRound"));
					vBubbleObject.transform.position = vcharacter.transform.position + new Vector3(8.15f, 25.75f, 0f); //move a little bit the teleport particle effect
				}
				...
				foreach (Transform child in vBubbleObject.transform)
				{
					...
					if (vRenderer != null && child.name.Contains("Body") && !child.name.Contains("BaseBody"))
					{
						//change the body color
						vRenderer.color = vNewBodyColor;
						
						if (vRenderer.sortingOrder < 10)
							vRenderer.sortingOrder = 2;
					}
					...
					else if (vTextMesh != null && child.name.Contains("Message"))
					{
						...
						child.GetComponent<MeshRenderer>().sortingOrder = 3;
	...
	void Update () 
	{
		...
		if (Physics.Raycast (ray, out hit) && Input.GetMouseButtonDown (0) && sidePanel.activeSelf == false) {
			//only return NPC
